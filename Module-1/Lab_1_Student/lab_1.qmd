...
4.  Create basic visualizations using both base R and `ggplot2` functions.
...
Every Quarto document begins with the YAML header that appears inside three dashes `---`. The header controls document level settings, such as the document type, the title, and the author. In this document, everything you see above the **Learning Objectives** section was specified in the YAML. 
...
**Helpful reminder:** Render at the start and as you go! You can and should run code chunks directly in RStudio as you work through this .qmd file, but you should also render the entire document fairly often to ensure that changes you've made will result in a working document.

### Rendering to PDF

To render your document to PDF, change the format option in the YAML from `html` to `pdf`.

In order to create PDFs you will need to install a recent distribution of TeX. We recommend the use of TinyTeX, which you can install by running the following command in the **Terminal**. To open the Terminal, select the tab just to the right of the Console. Simply type the following three words and hit return/enter.
